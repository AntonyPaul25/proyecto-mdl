...
public class GacController {
	...
	public ResponseEntity<?> ObtenerGAC(@Valid @RequestBody ClienteRequest clientrequest, BindingResult result) {
		...
		try {
			garantias = garantiaservice.listarGAC(headers, clientrequest).getDatos();
			garantias.stream().forEach(f -> f.setNumsolicitud(clientrequest.getNumsolicitud()));
			garantiaservice.saveAll(garantias);
